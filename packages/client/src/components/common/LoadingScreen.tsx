...
export function LoadingScreen() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#0A0A0A] animate-fade-in">
      <div className="relative mb-8">
        <svg viewBox="0 0 48 48" width="80" height="80" className="animate-mask-float">
          <defs>
      ...
      <div className="flex gap-1.5 mb-6">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className="w-2.5 h-2.5 rounded-full bg-[#8B0000] animate-bounce"
            style={{ animationDelay: `${i * 0.15}s` }}
          />
        ))}
      </div>

      <p className="text-[#8B0000] font-medium text-lg">{t('common.loading')}</p>

      <div className="max-w-md mt-8 text-center px-6 animate-fade-in-up">
        <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">{t('loading.tip')}</p>
        <p className="text-sm text-gray-400 leading-relaxed">{t(tipKey)}</p>
      </div>
    </div>
  );
}
