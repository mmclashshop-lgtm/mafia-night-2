    ...
    <>
      ...
      <aside
        className={`fixed top-0 right-0 z-50 h-full w-64
          ...
          border-l border-[#8B0000]/10
          transform transition-transform duration-300 ease-out
          ${isOpen ? 'translate-x-0' : 'translate-x-full'}
          flex flex-col`}
        dir="rtl"
      >
        <div className="flex items-center gap-3 p-5 border-b border-[#8B0000]/10">
          <img src={`${import.meta.env.BASE_URL}logo.svg`} alt="" className="w-8 h-8" />
          <div>
            <span className="font-bold text-sm block">{t('brand.name')}</span>
            {roomCode && (
              <code className="text-[10px] font-mono text-[#B22222]">{roomCode}</code>
            )}
          </div>
        </div>
        ...
        <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
          {NAV_ITEMS.map(({ path, icon: Icon, labelKey }) => (
            ...
            >
              {isActive(path) && (
                <span className="absolute right-0 top-1/2 -translate-y-1/2 w-0.5 h-6 bg-[#B22222] rounded-full" />
              )}
